## adr_004_teams_bot_architecture_for_llm_chat - DeepVault - Gordon architecture for LLM chat
> Date: 2026-04-10
> Status: Proposed
...
> Reminder: Keep the bot identity, `DeepVault - Gordon` payload handling, and rights checks aligned. For any UX/UI or frontend implementation work, use `logics/skills/logics-ui-steering/SKILL.md`.

# Overview
The chat surface should be `DeepVault - Gordon` rather than a fake user profile.
`DeepVault - Gordon` delivers messages to the backend.
The backend validates the user, checks access, queries the knowledge layer, and posts the answer back to Teams.

```mermaid
flowchart LR
    Current[No chat surface] --> Choice[DeepVault Gordon surface]
    Choice --> Teams[DeepVault - Gordon messages]
...
# Decision
Implement the first chatbot surface as `DeepVault - Gordon`.
...
# Follow-up work
- Register the `DeepVault - Gordon` app and bot
