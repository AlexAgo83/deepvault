...
- `npm run analyze:publish` now explicitly promotes the analyzed corpus into `public/live-corpus.json`, so the app reads a single published live snapshot instead of inferring from sidecar runtime artifacts.
