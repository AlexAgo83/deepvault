...
The product should focus on a local development surface with a clear validation role.
...
The local exploration UI is `DeepVault - Navy` and the local LLM chatbot is `DeepVault - Bishop`.
The delivery roadmap centers on a V1 local validation milestone.
...
- Future chat users who want answers grounded in SharePoint sources through `DeepVault - Bishop`.
...
- Out: a fake human profile, generic chat without permissions, a broad tenant-wide rollout, or hosted rollout mechanics.
...
- The product remains anchored on the local-first strategy brief for development and tests.
- The local development surface should prove the product value before any future expansion is considered.
...
- Any UX/UI or frontend implementation work for `DeepVault - Navy` or `DeepVault - Bishop` should use the `logics-ui-steering` skill before layout or styling decisions are made.
...
- `DeepVault - Navy` and `DeepVault - Bishop` should be the validation surfaces.
...
- The product remains reusable across local validation surfaces without reworking the core contract.
