## prod_001_local_first_development_and_test_strategy - Local-first development and test strategy
> Date: 2026-04-10
> Status: Proposed
...
The local app is the place to validate data flow, permissions, answer quality, and operational visibility before anything is promoted.
...
- Internal reviewers who want to inspect what the system ingested and how it responded.

# Goals
- Enable fast local iteration on explorer, chat, and sync behavior.
...
- In: a local companion app, local retrieval flow, local sync visibility, and provider-agnostic chat testing.
...
- The local surface should remain the primary proving ground for explorer and chat UX changes.
- Observability should be rich enough for debugging, but light enough to stay usable during rapid iteration.

# Success signals
- Engineers can validate new behavior locally without waiting for hosted deployment cycles.
...
```mermaid
flowchart LR
    Dev[Developer machine] --> App[Local companion app]
...
# Positioning
- This brief is the local experimentation and validation variant.
