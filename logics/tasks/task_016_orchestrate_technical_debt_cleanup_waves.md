...
- April 2026 settings persistence follow-up: changed provider API keys and the Entra client secret from session-only storage to browser-local persisted settings, added migration from legacy session entries, and realigned the Settings/README wording plus persistence tests.
- April 2026 local CI follow-up: added `npm run ci:local` to mirror the release validation lane with coverage, mock evaluation, Playwright browser install, and e2e in one command; removed the obsolete `role` prop from `BishopPanel`; and refreshed the workflow e2e selector plus README naming to match the shipped `AI View` / `Settings` surface.
