...
> Maintenance edit: completed the theme coverage wave and captured validation.
...
# Report
- Added `data-theme` synchronization on both `document.documentElement` and `document.body`, and exposed the theme toggle state via `aria-pressed`.
- Verified the persisted theme behavior with updated theme tests covering shell surface propagation.
